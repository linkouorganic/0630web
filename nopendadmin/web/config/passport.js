...
passport.deserializeUser(function(id, done) {
	...
	console.log(id.substring(0,1));//91
	console.log("------substring-----");
	console.log(id.substring(1));//91
	if (id.substring(0,1)== 'c'){
	pool.query("SELECT * FROM custom WHERE id = ? ",[id.substring(1)], function(err, rows){
		console.log(rows);
		console.log("xxxxxxxxxxcustom");
		done(err, rows[0]);
	});
	}
	else{
	pool.query("SELECT * FROM user WHERE id = ? ",[id.substring(1)], function(err, rows){
		console.log(rows);
		console.log("xxxxxxxxxxuser");
		done(err, rows[0]);
	});}
});
...
passport.use(
	...
	passport.use(
        ...
        function(req,Account, Password, done) { 
			...
			console.log("aaaaaqqqqqqqqqqqqqqqq");
