...
router.get('/', isLoggedIn,function(req, res, next) {
    ...
    if(pageNo>=(startPage+navSegments)){
	 ...
	 pool.query('SELECT count(*) as cnt FROM mydb.editpendord JOIN  mydb.`order` on ( mydb.`order`.OrdNo = editpendord.OrderDetail_Order_OrdNo) where mydb.`order`.createid=?  ', [req.user.createid], function(err, results) {
        if (err)throw err;
						...
						//console.log(suporddetailData);
							...
							//console.log(yetpendordData);
								...
								console.log(okpendordData);
									...
									var editpendordData;
									pool.query('SELECT * FROM mydb.editpendord JOIN  mydb.`order` on ( mydb.`order`.OrdNo = editpendord.OrderDetail_Order_OrdNo) where mydb.`order`.createid=? limit ?,? ',[req.user.createid,(pageNo-1)*linePerPage, linePerPage], function(err, results) {       
										if (err) {
												res.render('nosupplierpendedit', {user : req.user});
											}
											...
											if(results.length==0){
											    ...
											    res.render('supplierpendedit', { pageNo:pageNo, totalLine:totalLine, totalPage:totalPage, startPage:startPage, linePerPage:linePerPage, navSegments:navSegments,productData:productData,orderData:orderData,orderdetailData:orderdetailData,customData:customData,supplierData:supplierData,suporddetailData:suporddetailData,yetpendordData:yetpendordData,okpendordData:okpendordData,editpendordData:results,user : req.user});    
