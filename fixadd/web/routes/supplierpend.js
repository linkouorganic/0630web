					...
					//console.log(supplierData);
						...
						//console.log(suporddetailData);
							...
							pool.query('select * from yetpendord',  function(err, results) {       
								...
								var okpendordData;
								pool.query('SELECT * FROM mydb.okpendord limit ?, ?', [(pageNo-1)*linePerPage, linePerPage], function(err, results) {       
									if (err) {
												res.render('nosupplierpend', {user : req.user});
											}
											...
											if(results.length==0){
												...
												res.render('supplierpend', {pageNo:pageNo, totalLine:totalLine, totalPage:totalPage, startPage:startPage, linePerPage:linePerPage, navSegments:navSegments,productData:productData,orderData:orderData,orderdetailData:orderdetailData,customData:customData,supplierData:supplierData,suporddetailData:suporddetailData,yetpendordData:yetpendordData,okpendordData:results,editpendordData:editpendordData,user : req.user});    
											}
