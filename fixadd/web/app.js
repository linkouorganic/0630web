...
var supplierSearch = require('./routes/supplierSearch');//訂單清單搜尋
var supplierordedit = require('./routes/supplierordedit');//訂單編輯
var supplierstatus = require('./routes/supplierstatus');//訂單狀況
var supplierstatusSearch = require('./routes/supplierstatusSearch');//訂單狀況
...
var supplieruserSearch = require('./routes/supplieruserSearch');
var supplieruserpend = require('./routes/supplieruserpend');
//供應商各家負責人新增產品
...
var shoppingAddPro = require('./routes/shoppingAddPro');
...
// 新增商品
var carts = new Array();
var shopQty = new Array();



app.get('/add-to-cart/:id',isLoggedIn, function(req, res, next){
var productId = req.params.id;
var quantity=req.param("quantity");
	console.log("quantity");
	console.log(quantity);
carts.push(productId);
/*var result=carts.filter(function(element, index, arr){
    return carts.indexOf(element)=== index;
});*/
if(quantity==undefined){
	shopQty.push(1);
}
else{
	shopQty.push(quantity);
}
...
req.session.shopQty=shopQty;
res.redirect('back');

});

// 刪除商品
app.get('/del/:id',isLoggedIn, function(req, res, next){
    for(var i=0;i<carts.length; i++){
    if( req.params.id==carts[i] ){
...
//Post
app.post('/cartdata', function(req, res, next){
        console.log('啊啊啊啊啊啊');
...
app.use('/shoppingAddPro', shoppingAddPro);
